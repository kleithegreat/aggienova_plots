...
const ColorBandSelector: React.FC = () => {
  ...
  const handleFirstColorChange = (event: React.ChangeEvent<{ value: unknown }>) => {
    setFirstColor(event.target.value as ColorOption);
  };

  const handleSecondColorChange = (event: React.ChangeEvent<{ value: unknown }>) => {
  ...
  return (
    <div className="flex items-center space-x-2">
      <FormControl sx={{ minWidth: 120 }} size="small">
        <InputLabel id="first-color-label">First Color</InputLabel>
        <Select
      ...
      <span>-</span>
      <FormControl sx={{ minWidth: 120 }} size="small">
        <InputLabel id="second-color-label">Second Color</InputLabel>
        <Select
